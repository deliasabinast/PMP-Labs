...
//tried to add 1 more card for each player
val player1Card2 = Chain(player1Card, player2Card (card: Int) => discrete.Uniform(cards.filter(_ != card):_*))
val player2Card2 = Chain(player1Card, player2Card, player1Card2 (card: Int) => discrete.Uniform(cards.filter(_ != card):_*))

val player1Bet1 = RichCPD(player1Card, OneOf(5, 4, 3) -> Flip(0.9),* -> Flip(0.4))
// Player 1 is more likely to bet with a higher card,
// but will sometimes bet with a lower card to bluff
 // ×××Change this for part (c)×××
val player2Bet =RichCPD(player2Card, player1Bet1,(OneOf(5, 4), *) -> Flip(0.9),(*, OneOf(false)) -> Flip(0.5),(*, *) -> Flip(0.1))
val player1Bet2 =Apply(player1Card, player1Bet1, player2Bet,(card: Int, bet11: Boolean, bet2: Boolean) =>!bet11 && bet2 && (card == 5 || card == 4))
